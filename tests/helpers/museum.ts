...
import { buildDoorPieces, buildWallPieces, type WallPiece } from '../../src/world/wallGeometry';
import { AREAS, DOORWAYS, WALL_THICKNESS } from '../../src/data/layout';

/** 実レイアウトから壁の当たり判定を組み立てる（RoomBuilder と同じ手順） */
export function museumCollision(options: { doors?: boolean } = {}): Collision {
  const collision = new Collision();
  const pieces: WallPiece[] = [];
  for (const area of AREAS) pieces.push(...buildWallPieces(area, DOORWAYS));
  if (options.doors) pieces.push(...buildDoorPieces(DOORWAYS));
