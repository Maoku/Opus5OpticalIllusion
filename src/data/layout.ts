...
export const AREAS: readonly AreaDefinition[] = [
  ...
  // 明順応の落差を作るため、天井も低く抑える。南口から大広間の西へ抜けられる
  {
    id: 'roomDAlcove',
    room: 'opus',
    min: [-14, -28],
...
export const DOORWAYS: readonly Doorway[] = [
  ...
  // roomDNorth <-> roomDAlcove（暗い小部屋の主入口）
  { min: [-4.5, -24], max: [-3.5, -20], height: DOOR_HEIGHT },
  /*
   * roomDAlcove <-> roomD（アルコーブの南口）。
   *
   * これが無いと大広間の西半分が袋小路になる。大広間への開口は
   * x∈[-3,5] の 1 箇所だけで、その真ん中に D2 の縮む廊下（x∈[-1.4,1.4]）が
   * 刺さっているため、西へ回れるのは幅 1.6m の隙間しかない。
   * 結果 D5「横たわる影」と D1「二つの真実」が壁の裏に隠れていた。
   * D6 を見た後そのまま南へ抜けられるようにして、袋小路を順路に変える。
   *
   * 東端（x∈[-6.5,-4.5]）に置くのは D1 のため。視点 A（南 → 北）が背負う
   * 北壁は x∈[-12.4,-6.7] に見えており、そこへ開口が掛かると字が読めない。
   * 幅を 2m に抑えてあるのはアルコーブの囲まれた感じを残すため。
   * 明順応の落差は照明プロファイルが担っているので、開口が増えても崩れない。
   */
  { min: [-6.5, -28.5], max: [-4.5, -27.5], height: DOOR_HEIGHT },
];
