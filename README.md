...
館内には 20 の展示が並び、それぞれ「立って見るべき場所」から覗くと錯視が成立します。
...
（[Docs/CREDITS.md](Docs/CREDITS.md)）。唯一の配信ファイルである「嘘つきの影」の
塊も、自前のシルエット定義からビルド時に彫り出したものです。
...
| Room D | Opus 棟：絵にできない錯視 | 縞の下の嘘 / 聞こえる衝突 / 後ろの正面 / 嘘つきの影 / 縮んでいく部屋 / 二つの真実 |

Room D は、静止画では成立しない — 移動・音・時間・視線を使う錯視を集めた棟です。
...
src/
  ...
  data/        部屋と通路のレイアウト定義
  exhibits/    展示 20 件と共通実装（パネル / 不可能図形 / 視線投影 / 種明かしカメラ /
               文字の点群化 / 2 視点アナモルフォーシス / 視界判定 / visual hull）
  viewpoint/   ViewSpot（立ち位置の誘導とカメラ固定）
  player/      移動と視点操作
  ui/          HUD・ヒント・展示一覧・設定・仮想パッド
  i18n/        日本語 / 英語の辞書
tests/         Vitest（レイアウト整合・配置検証・入力・UI ほか）
tools/         visual hull の生成（嘘つきの影）／フォントのサブセット化
Docs/          計画・改修計画・QA チェックリスト・クレジット
```

`public/` に置いてあるのは `models/shadowHull.glb` だけです。これも外部から
持ち込んだ素材ではなく、シルエットの定義からビルド時に彫り出した中間成果物で、
`npm run build:hull` で作り直せます（ファイルが無ければ実行時に彫り直します）。
